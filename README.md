...
### Routes are generated by the server

A route is the ordered list of stations a team walks. The Worker generates it,
stores it on the team row, and hands it over on join and on every sync:

```json
[{ "beat": 3, "variant": "B" }, { "beat": 4, "variant": "A" }, ...]
```

**Position** is the index — position 1 is played first, and it is what
`completions.level` counts and what the app numbers *Level 1 of 10*. **Beat** is
which of the ten story levels sits at that position. Under story order the two
are the same number; under a staggered order they come apart. Keeping them
separate is what lets the order change without the sequence rule, the progress
keys or the UI knowing anything about it.

It used to be a pure function of the team code, computed in the bundle. Two
things were wrong with that. A route derivable on one phone is derivable for
*every* team by anyone who opens devtools, which is a map of where to camp. And
changing an assignment meant rebuilding and redeploying the web app, which is
not a thing to be doing on the morning of an event.

`src/lib/hunt.js` still has `localRouteFor` as a **fallback**, used only when
there is no Worker at all, so a rehearsal on one laptop still runs.

Two orders:

| Order | What it does | Cost |
| --- | --- | --- |
| `story` *(default)* | beats 1→10 in sequence | all fifteen teams open at beat 1, so ~7 teams per station in the first five minutes |
| `stagger` | rotates each team's starting beat | at most 2 teams per opening station — but the story arrives out of sequence |

`story` is the default because the ten beats are a narrative and reading chapter
seven first is a mistake, not a design. Whether the opening crush matters more
than the narrative is a question about your campus, so it is a flag:

```bash
npm run db:teams -- --stagger        # at seed time
```

or the **Regenerate routes** control in `/admin`. Regenerating is refused once
any team has recorded a completion — rerouting mid-run sends a team to a station
they have already visited, and their recorded positions would then name the
...
**Neither secret may appear in `wrangler.jsonc`.** `wrangler deploy` uploads the
`vars` in that file and a var *overwrites* a secret of the same name — it is not
overridden by it. A dev placeholder sitting in `vars` therefore does not get
replaced by `wrangler secret put`; it silently replaces the real secret on the
next deploy and puts a value from a public repo in front of every team's pin.
This was found by deploying and testing it, not by reading about it.

Local values go in `apps/api/.dev.vars` (git-ignored, copy `.dev.vars.example`),
which `wrangler dev` reads and `wrangler deploy` never uploads. Deployed values
come only from `wrangler secret put`, and the Worker refuses to run without them
rather than falling back to a default.

Set them before the event, not during: rotating `TEAM_SECRET` signs every phone
out, and fifteen teams re-entering a pin mid-hunt is not a good afternoon.

After `secret put`, give the rollout a few seconds before testing — each one
creates a new Worker version, and requests during the rollout can hit either.
...
`npm run db:teams` seeds the roster, generates the routes and prints the
code/pin slips to hand out. It upserts, so re-running before the event keeps the
progress *and the route* of a team that has already started; pass `--repin` to
roll new pins, `--stagger` for a staggered order. `npm run db:seed` is unrelated
...
apps/web/                       -> Cloudflare Pages
  ...
  src/
    ...
    lib/hunt.js                 the 10x2 levels - the content you edit
    lib/hunt.js:localRouteFor   fallback route, used only with no Worker
...
apps/api/                       -> Cloudflare Workers (shared progress + analytics)
  wrangler.jsonc                Worker name, D1 binding, non-secret vars
  .dev.vars.example             template for local TEAM_SECRET / ADMIN_TOKEN
  src/
    index.js                    CORS, security headers, routing, error shape
    routes/teams.js             join, sync, leaderboard
    routes/admin.js             roster, matrix, level override, reset
...
| `TEAM_SECRET` | `.dev.vars` locally, **`wrangler secret put`** deployed | none | signs team tokens. Never put it in `wrangler.jsonc` — a var there overwrites the secret on deploy. Unset is a 503 |
| `ADMIN_TOKEN` | `.dev.vars` locally, **`wrangler secret put`** deployed | none | guards `/api/admin`, which lists every pin. Unset is a 503, not open |
